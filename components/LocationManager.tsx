    ...
    return (
        <div className="space-y-6 animate-fade-in max-w-6xl mx-auto pb-20">
            <div className="flex items-center justify-between">
                ...
                <div className="bg-blue-900/20 text-blue-300 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 border border-blue-800">
                    <ShieldCheck size={16} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {locations.map((loc) => (
                    <div key={loc.id} className="group relative bg-slate-900 rounded-3xl p-6 border border-slate-800 shadow-xl transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">

                        {/* Header / Name */}
                        <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${loc.lat === 0 ? 'bg-amber-900/30 text-amber-500' : 'bg-blue-900/30 text-blue-400'
                                    }`}>
                                    <Navigation size={24} className={loc.lat === 0 ? '' : 'rotate-45'} />
                                </div>
                                <div>
                                    <h3 className="font-bold text-lg text-white">{loc.name}</h3>
                                    <code className="text-[10px] text-slate-400 bg-slate-800 px-2 py-0.5 rounded font-mono">{loc.id}</code>
                                </div>
                            </div>
                            <div className={`w-3 h-3 rounded-full ${loc.active ? 'bg-green-500 shadow-lg shadow-green-500/50' : 'bg-slate-700'}`} />
                        </div>
                        ...
                        <div className="space-y-3 bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50">
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span className="font-mono font-bold text-slate-200">{loc.lat.toFixed(6)}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-slate-400">خط الطول (Lng)</span>
                                <span className="font-mono font-bold text-slate-200">{loc.lng.toFixed(6)}</span>
                            </div>
                            <div className="pt-2 mt-2 border-t border-slate-700 flex justify-between items-center">
                                ...
                                <span className="text-xs font-bold bg-indigo-900/30 text-indigo-300 px-2 py-1 rounded-lg">
                                    {loc.radius} متر
                                </span>
                            </div>
                        </div>
                        ...
                        {loc.lat === 0 && (
                            <div className="mt-4 flex items-center gap-2 text-amber-400 text-xs font-bold bg-amber-900/10 p-3 rounded-xl border border-amber-900/30">
                        ...
                        {/* Hover Actions */}
                        <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 bg-slate-900/90 backdrop-blur-sm rounded-b-3xl border-t border-slate-800 flex justify-center">
                            <a
                                href={`https://www.google.com/maps/search/?api=1&query=${loc.lat},${loc.lng}`}
                                target="_blank"
                                rel="noreferrer"
                                className={`text-xs font-bold flex items-center gap-1 ${loc.lat === 0 ? 'pointer-events-none text-slate-400' : 'text-blue-400 hover:text-blue-300 hover:underline'}`}
                            >
                                فتح في خرائط Google
                            </a>
                        </div>
